...
import { updateJobRequest } from 'store/slices/job';
...
const Job = ({ open, close, item }) => {
  ...
  const [endDateVar, setEndDateVar] = useState(true);
  ...
  const initialValues = {
    ...
    start: item?.start ? parseISO(item.start) : null,
    end: item?.end ? parseISO(item.end) : null,
    work_time_h: item?.work_time?.split(/\s+/)[0],
    work_time_m: item?.work_time?.split(/\s+/)[1],
    task_date: item?.task_date ? parseISO(item.task_date) : null,
    service_unit: item?.service_unit,
    quote: item?.quote,
    critical: item?.critical,
    task_details: item?.task_details,
    short_descrption: item?.short_descrption,
    long_descrption: item?.long_descrption,
  ...
  const onSubmit = async (values, { resetForm }) => {
    ...
    try {
      ...
      const res = await dispatch(updateJobRequest(data));
            ...
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} sm={6}>
                <FormControl fullWidth error={formik.touched.work_time_h && Boolean(formik.errors.work_time_h)}>
                  ...
                  {formik.touched.work_time_h && formik.errors.work_time_h && <FormHelperText>{formik.errors.work_time_h}</FormHelperText>}
                </FormControl>
              </Grid>
              <Grid item xs={12} sm={6}>
                <FormControl fullWidth error={formik.touched.work_time_m && Boolean(formik.errors.work_time_m)}>
                  ...
                  {formik.touched.work_time_m && formik.errors.work_time_m && <FormHelperText>{formik.errors.work_time_m}</FormHelperText>}
                </FormControl>
              </Grid>
              <Grid item xs={12} sm={6}>
                <LocalizationProvider dateAdapter={AdapterDateFns}>
              ...
              {/* <Grid item xs={12} sm={4} container alignItems="flex-start">
                <Button variant="contained" sx={{ my: 1, ml: 1 }} onClick={handleGetLocation}>
