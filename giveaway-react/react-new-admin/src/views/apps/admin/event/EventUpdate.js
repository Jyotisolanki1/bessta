...
import { updateEventRequest } from 'store/slices/events';
...
const EventUpdate = ({ open, close, item }) => {
  ...
  const validationSchema = yup.object({
    ...
    scheduleDate: yup
      ...
      .typeError('Invalid Schedule date')
    ...
    price: yup
      ...
      .min(2, 'Price 1 must be at least 2 characters')
      .max(100, 'Price 1 must be at most 100 characters'),
    price2: yup
      .string()
      .matches(/^[^\s]+(\s[^\s]+)*$/, 'Multiple consecutive spaces are not allowed, and spaces are not permitted at the beginning or end')
      .min(2, 'Price 2 must be at least 2 characters')
      .max(100, 'Price 2 must be at most 100 characters'),
    price3: yup
      .string()
      .matches(/^[^\s]+(\s[^\s]+)*$/, 'Multiple consecutive spaces are not allowed, and spaces are not permitted at the beginning or end')
      .min(2, 'Price 3 must be at least 2 characters')
      .max(100, 'Price 3 must be at most 100 characters')
  });

  const initialValues = {
    name: item?.name,
    discription: item?.discription,
    scheduleDate: item?.scheduleDate ? parseISO(item.scheduleDate) : null,
    price: item?.prizes[0]?.description,
    price2: item?.prizes[1]?.description,
    price3: item?.prizes[2]?.description,
    image: null
  };
  const onSubmit = async (values, { resetForm }) => {
    ...
    try {
      ...
      // formData.append('prizes[0][quantity]', values.quantity);
      // formData.append('prizes[0][reserves]', values.reserves);
      ...
      // console.log()
      const res = await dispatch(updateEventRequest(formData));
      if (res?.success === true) {
        setLoading(false);
        resetForm();
        dispatch(
          openSnackbar({
            open: true,
            message: 'Draw details updated successfully',
            variant: 'alert',
            alert: {
              color: 'success'
            },
