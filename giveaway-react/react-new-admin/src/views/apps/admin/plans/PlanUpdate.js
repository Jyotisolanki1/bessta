...
import { updatePlanRequest, getCategories } from 'store/slices/plans';
...
const PlanUpdate = ({ open, close, item }) => {
  ...
  const validationSchema = yup.object({
    ...
    entries: yup.string().required('Entries is is required').notOneOf(['0'], 'Price cannot be zero')
    // price: yup.number().required('Price is required').nullable().positive('Price must be a non-negative number')
  });
    ...
    try {
      const res = await dispatch(
        updatePlanRequest({
          ...
          mostPopuler: values.mostPopuler,
          price: values.price
        ...
        <DialogContent>
          <DialogContentText id="alert-dialog-slide-description" sx={{ pt: 1 }}>
            <Grid container spacing={3}>
              ...
              <Grid item xs={12} sm={6}>
                <TextField id="category" name="price" label="Category" value={formik.values.category} fullWidth disabled />
              </Grid>
              <Grid item xs={12} sm={6}>
                <FormControl fullWidth error={formik.touched.intervalType && Boolean(formik.errors.intervalType)}>
                  ...
                  <Select
                    ...
                    label="Most Popular"
                    name="mostPopuler"
                    value={formik.values.mostPopuler || 'select'}
                    onChange={formik.handleChange}
                    onBlur={formik.handleBlur}
                    sx={{ width: '100%' }}
                  >
