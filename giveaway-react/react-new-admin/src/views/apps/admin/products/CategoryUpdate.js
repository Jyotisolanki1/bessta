...
import { updateCategoryRequest } from 'store/slices/products';
...
const Employee = ({ open, close, item }) => {
  const dispatch = useDispatch();

  const imageInputRef = useRef(null);
  const Avatar1 = `${imgPath}${item?.icon}`;
  ...
  const onSubmit = async (values, { resetForm }) => {
    ...
    try {
      ...
      const res = await dispatch(updateCategoryRequest(formData));
      if (res?.success === true) {
        setLoading(false);
        resetForm();
        dispatch(
