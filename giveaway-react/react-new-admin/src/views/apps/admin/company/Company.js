...
import { updateCompanyRequest } from 'store/slices/company';
...
const Company = ({ open, close, item }) => {
  ...
  const validationSchema = yup.object({
    ...
    // password: yup
    //   .string()
    //   .test('password-validity', <FormattedMessage id="password-error-type" />, (value) => {
    //     // Check if the value exists before applying the regex
    //     if (value) {
    //       // Apply password validation regex
    //       return /^(?=.*[a-z])(?=.*[A-Z])(?=.*\d)(?=.*[@$!%*?&])[A-Za-z\d@$!%*?&]+$/.test(value);
    //     }
    //     // If value is empty, validation passes since password is optional
    //     return true;
    //   })
  ...
  const onSubmit = async (values, { resetForm }) => {
    ...
    try {
      ...
      const res = await dispatch(updateCompanyRequest(formData));
                ...
                <Grid container spacing={2} alignItems="center">
                  <Grid item>
                    {selectedImage ? (
                      ...
                      <Avatar alt="Company Logo" src={Avatar1} sx={{ height: 120, width: 120, borderRadius: '8px' }} />
                    )}
                  </Grid>
