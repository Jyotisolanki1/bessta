...
const ProductDetail = () => {
  ...
  const { suggestedProduct,status } = useSelector((state) => state.storeAction);
  // console.log(status)
  ...
  useEffect(() => {
    if (ProductDetails) {
      ...
      setProdRating(ProductDetails[0]?.avgRating ? Number(ProductDetails[0]?.avgRating) : 0);
    }
  }, [ProductDetails[0]?.category?._id, ProductDetails[0]?.avgRating]);
  ...
  const GetProductDetails = async () => {
    ...
    if (response.success) {
      ...
      // Set initial price object
      const prizeFilter = response.data.find((eachItem) => eachItem._id === id);
      setPriceObject(prizeFilter?.variableProducts[0]);
    }
  };
  ...
  const onSizeChange = (e) => {
    const selectedVariableId = e.target.value;
    const filterProductDetails = variableProductDetails.filter((eachItem) => eachItem.variableId === selectedVariableId);
    setPriceObject(filterProductDetails[0]);
    setSizeStore(filterProductDetails[0]?.price);
  };
  const handleAddToCart = async () => {
    if (!priceObject || !priceObject.variableId) {
  ...
  return (
    <div>
              ...
              <div className="product-details__content" style={{ width: '100%' }}>
                {loading ? (
                  ...
                  <div className="product-details__top">
                    ...
                    <div className="product-details__price"> {priceObject&& `${priceObject?.price ? priceObject?.price : sizeStore}`}</div>
                  </div>
                )}
                ...
                {loading ? (
                  ...
                  <div style={{ display: 'flex' }}>
                    <Rating name="read-only" value={prodRating} readOnly precision={0.5} />
                    &nbsp; &nbsp;{prodRating} Ratings
                  </div>
                )}
                ...
                {loading ? (
                  ...
                  <div className="product-details__excerpt">
                    ...
                    <p className="product-details__excerpt-text2">
                      {ProductDetails[0]?.stock !== 0 ? <>Available in store {ProductDetails[0]?.stock}</> : <h1 style={{"color":"red"}}>Out Of Stock</h1>}
                    </p>
                  </div>
                )}
                  ...
                  <div className="col-lg-6 col-md-12">
                    ...
                    <div className="custom-select">
                      ...
                      {loading ? (
                        ...
                        <>
                        <select id="size" name="size" className="form-control" onChange={onSizeChange}>
                        <option value={0} selected={sizeStore === ""}>
                          Choose Size
                        </option>
                ...
                status === 'enable' && ProductDetails[0]?.stock !== 0 ?<div onClick={handleAddToCart}>
      ...
      <div style={{ marginLeft: '3%' }}>
        ...
        {more && (
          <p style={{ fontWeight: '600', cursor: 'pointer', paddingBottom: '1%' }} onClick={handleMore}>
            View more review ...
          </p>
        )}
      </div>
